...
public class Main : CW_Addon<Main>, IReloadable
{
    ...
    public void Reload()
    {
        foreach (var library in BaseExtendedLibrary.libraries) library.Reload();
    }
...
#if 一米_中文名
        CN_NameGeneratorLibrary.SubmitDirectoryToLoad(Path.Combine(Declaration.FolderPath, "name_generators"));
        WordLibraryManager.SubmitDirectoryToLoad(Path.Combine(Declaration.FolderPath, "word_libraries"));
#endif
